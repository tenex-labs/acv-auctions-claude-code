...
Extract `inspection-desk-task1-migrated.zip` into a new folder after the backup succeeds. Carry over your own workshop notes, complete spec-to-build skill and reviewed project rules. Run `npm run setup` and `npm run preflight` in the new folder. Keep the previous project available for comparison.
